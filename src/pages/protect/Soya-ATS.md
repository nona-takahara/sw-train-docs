...
|3|R|-|-|ATSによる制動が常用最大制動の場合は常に制動条件を成立させる。<br>ATSによる制動がATS非常制動の場合は列車速度の絶対値が 1 km/h 程度を上回った場合に制動条件を成立させる|
|2|R|-|-|ATSによる制動が常用最大制動の場合は常に制動条件を成立させる。<br>ATSによる制動がATS非常制動の場合は列車速度の絶対値が 1 km/h 程度を上回った場合に制動条件を成立させる|
|1|Ea|-|-|常に制動条件を成立させる。他の信号においてATSによる制動がATS常用最大制動の設定の車両もATS非常制動とし、復帰操作を必須とする|
|0|E|-|-|常に制動条件を成立させる。他の信号においてATSによる制動がATS常用最大制動の設定の車両もATS非常制動とし、復帰操作を必須とする|
...
また、これに加えてT信号照査速度以上の速さで後退した場合は、ATS非常制動・ATSブザーの5秒以下鳴動をおこなう。
...
Y以上の信号、Tt信号、YYt信号のいずれかを受信したら確認扱いを終了し、通常時のアルゴリズムに戻る。ATSを終了した場合も通常時のアルゴリズムに戻る。
...
stateDiagram-v2
    ...
    [*] --> off
    off --> on: ATS起動条件 & 運転台起動
    on --> off: NOT(ATS起動条件 & 運転台起動)
    state on {
        direction LR
        ...
        [*] --> if_state1
        if_state1 --> m1: [非常運転 = OFF]
        if_state1 --> m3 : [非常運転 = ON]

        m1 --> m3: 非常運転ON
        m2 --> m3: 非常運転ON
        m3 --> m1: 非常運転OFF

        m1 --> m2: 確認運転条件 & 確認ボタン押下
        m2 --> m1: 確認運転終了条件
    }
```
